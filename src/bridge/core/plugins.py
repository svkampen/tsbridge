...
class PluginLoader:
    ...
    def __init__(
        ...
        blacklist: Optional[List[Plugin]] = None,
    ):
    ...
    def parse_depends(self, path: Path) -> None:
        ...
        with open(main_file) as plugin:
            for line in plugin:
                if "* noload" in line:
                    logger.info(
                        f"Not loading plugin {path.stem} " "due to noload directive"
                    )
        ...
        for plugin, deps in self.graph.items():
            ...
            if blacklisted_deps:
                ...
                raise DependencyError(
                    f"Plugin {plugin.stem} requires {', '.join(deps)},"
    ...
    def load_all(self) -> List[ModuleType]:
        ...
        while self.graph:
            satisfied_plugins = {
                plugin: deps for plugin, deps in self.graph.items() if not deps
            }
