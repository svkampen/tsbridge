...
class IRCUserFactory:
    ...
    def user(
        ...
        nick: str,
        user: str = "user",
        host: str = "bridge.segfault.party",
        modes: str = "+iwB",
    ) -> IRCUser:
        return IRCUser(
            nick,
...
class IRCDProto(Proto):
    ...
    def parse(self, msg: str) -> Tuple[str, str, List[str]]:
        ...
        tokens = (
            msg.split()
            if not tr_start
            else [*msg[:tr_start].split(), msg[tr_start + 1 :]]
        )
    ...
    def find_user(self, name_or_uid: str) -> Optional[IRCUser]:
    ...
    async def add_user(self, *args: Any, **kwargs: Any) -> IRCUser:
    ...
    async def join_channel(self, user: IRCUser, channel: str) -> None:
        """Make a user join a channel."""
        await self.write(f"SJOIN {itime()} {channel} +nt :{user.uid}")

    async def send_user(self, u: IRCUser) -> None:
        """Announce a user on the link."""
        await self.write(
            f"UID {u.nick} {u.hopcount} {u.connect_ts} {u.user} {u.host} {u.uid} {u.svstamp} {u.usermodes} {u.vhost} {u.clkhost} {u.ip} :{u.real}"
        )
    ...
    async def start_link(self) -> None:
        ...
        await self.write(
            f"PROTOCTL EAUTH={self.cfg['linkname']},,,Bridge-0.1 SID={self.cfg['sid']}"
        )
        await self.write(
            f"PROTOCTL NOQUIT NICKv2 SJOIN SJOIN2 SJ3 CLK TKLEXT TKLEXT2 NICKIP ESVID MLOCK EXTSWHOIS TS={itime()}"
        )
