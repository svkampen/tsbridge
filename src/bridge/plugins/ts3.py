...
class TS3Proto(Proto):
    async def start(
        ...
        sq_user, sq_password, sq_host, sq_port = map(
            instance_cfg.get, ["sq_user", "sq_password", "sq_host", "sq_port"]
        )
        ...
        self.client = await ServerQueryClient.create(
            sq_user, sq_password, sq_host, int(sq_port)
        )
    ...
    async def handle_notify_part(self, data: Dict) -> None:
        ...
        if clid in self.clid_users:
            ...
            logger.warning(
                f"Unable to find client with id {clid} in user list! (maybe a SQ client)."
            )

    async def handle_notify_text(self, data: Dict) -> None:
        """Handle a text notification"""
        message = Message(
            user=data["invokername"], text=data["msg"], channel=self.channel
        )
        if message.user in self.blacklisted_users:
            return
    ...
    async def _handle_message(
        ...
        try:
            await self.client.request(
                f"clientupdate client_nickname=Bridge-{meta.from_instance}-{message.user}"
            )
        except SQError as e:
            if e.errno != 513:  # nick already in use
                raise
        await self.client.request(
            f"sendtextmessage targetmode=2 target={to_channel} msg={txt}"
        )
