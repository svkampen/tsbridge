...
from typing import Any, Callable, Sequence, List, Dict, Awaitable
import logging
import json
import inspect
import asyncio
...
class MinecraftProto(Proto):
    ...
    async def generate_msg_json(
        self, from_instance: str, message: Message, handle_attachments: bool = True
    ) -> Any:
        ...
        for attachment in message.attachments:
            if isinstance(attachment, Photo) and self.img_host:
                url = (
                    await self.img_host.put(attachment.get())
        ...
        if message.reply_to is not None:
            ...
            if isinstance(message.reply_to, Message):
                reply_fmt = await self.generate_msg_json(
                    origin, message.reply_to, False
                )
                fmt.append(
                    {
                        "text": "[REPLY] ",
                        "color": "dark_red",
                        "hoverEvent": {"action": "show_text", "contents": reply_fmt},
                    }
                )
